...
class BSNode:
    ...
    def verify_DSSD(self, x_draft: torch.Tensor, q_values: List[float], gamma: int) -> Tuple[int, int, torch.Tensor, torch.Tensor, int, int]:
        ...
        # print("x_draft", x_draft)
        # exit(0)

        # print("\n[调试输入状态]")
        # print("prefix_len:", prefix_len)
        # print("gamma:", gamma)
        # print("x_draft 前20个 token:", x_draft[0, :20].tolist())
        # print("模型是否在 eval 模式:", not self.model.training)
        ...
        print(f"  p_logits.shape: {p_logits.shape} (从位置{prefix_len-1}到{prefix_len+gamma-1}，共{gamma+1}个分布)")
        print(f"  p_probs.shape: {p_probs.shape}")
        print(f"  温度: {self.args.temperature}")
        
        # 打印前3个分布的Top-5 tokens
        for i in range(min(3, gamma+1)):
            top5_probs, top5_ids = torch.topk(p_probs[i], 5)
            print(f"  P_{i+1}的Top-5概率:")
            for prob, tok_id in zip(top5_probs, top5_ids):
                print(f"    token_id={int(tok_id):5d}, prob={prob.item():.6f}")
        # exit(0)
        ...
        for i in range(gamma):
            ...
            r = torch.rand(1).item()  # 生成随机数
            threshold = min(1.0, p_i / q_i)  # 接受阈值
            accept = r <= threshold  # 是否接受
            
            print(f"  Token {current_j}/{gamma}:")
            print(f"    tok_id={tok_id}, q_i={q_i:.6f}, p_i={p_i:.6f}")
            print(f"    p_i/q_i={p_i/q_i:.6f}, threshold=min(1, {p_i/q_i:.6f})={threshold:.6f}")
            print(f"    random r={r:.6f}, {'✓ 接受' if accept else '✗ 拒绝'}")
...
def generate_DSSD(uav_node: UAVNode, bs_node: BSNode, input_ids: torch.Tensor, tokenizer: AutoTokenizer, args: argparse.Namespace) -> Tuple[str, float, float, float, float, int, int, float, float]:
    ...
    with tqdm(total=max_total_len, desc="DSSD: Distributed Split Speculative Decoding") as pbar:
        ...
        while prefix.shape[1] < max_total_len:
            ...
            # # 打印verify结果
            print(f"[Verify结果]")
            print(f"  j: {j}, flag: {flag}, correct_num: {correct_num}, reject_num: {reject_num}")
            if pj is not None:
                try:
                    topk = min(5, pj.numel())
                    vals, idx = torch.topk(pj, k=topk)
                    top_items = [(int(idx[i].item()), float(vals[i].item())) for i in range(topk)]
                    print(f"  pj.shape: {tuple(pj.shape)}, top{topk}: {top_items}")
                except Exception:
                    print(f"  pj.shape: {tuple(pj.shape)}")
            else:
                print("  pj: None")
            if xj is not None:
                try:
                    print(f"  xj: {int(xj.item())}")
                except Exception:
                    print(f"  xj: {xj}")
            else:
                print("  xj: None")
            ...
            if flag == 1:
                ...
                print(f"      基站返回: j={j} (gamma+1), xj={int(xj.item())}")
                xj_text = tokenizer.decode([int(xj.item())], skip_special_tokens=True)
                ...
                # 下行传输内容：j=gamma+1（int） + xj（token）
                down_bytes = 4 + xj.numel() * 4  # j占4字节，xj占4字节
                print(f"      下行传输字节数: {down_bytes}B (j: 4B + xj: 4B)")
                ...
                # 下行传输内容：j（int） + P_j分布（tensor）
                down_bytes = 4 + tensor_nbytes(pj)  # j占4字节，pj占V*4字节（V为词表大小）
                # print(f"      下行传输字节数: {down_bytes}B (j: 4B + P_j: {tensor_nbytes(pj)}B)")
