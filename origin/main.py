...
def load_mini_boolq(local_path, n_samples=50, seed=42):
    """
    从本地 BoolQ 数据集中随机抽取 n 条数据。
    BoolQ 是 yes/no 判断题，非常适合 correctness regression。
    """
    dataset = load_dataset(
        path=local_path,
        split="validation"   # BoolQ 通常用 validation 作测试
    )
...
def evaluate_on_mini_boolq(uav_node, bs_node, tokenizer, args,
                           local_path="/home/lym/sijia/DSSD-Efficient-Edge-Computing/benchmark/boolq",
                           n_samples=50):
    """
    使用本地 BoolQ 构建 mini-dataset，测试 speculative decoding 是否正确。
    """
    dataset = load_mini_boolq(local_path, n_samples=n_samples)

    correct_base = 0
    correct_dssd = 0
    correct_dsp = 0

    print("\n=== Running Mini-BoolQ correctness test ===")

    for item in tqdm(dataset, desc="Mini-BoolQ Testing"):
        question = item["passage"] + "\nQuestion: " + item["question"]
        gt = "yes" if item["answer"] is True else "no"

        # ===== Baseline =====
        input_ids = tokenizer(question, return_tensors="pt").input_ids.to(args.device_2)
        base_out, _, _ = normal_generate(bs_node.model, tokenizer, input_ids, args.device_2, args)
        base_text = to_text(base_out, tokenizer)
        base_pred = extract_bool_answer(base_text)
        if base_pred == gt:
            correct_base += 1

        # ===== DSSD =====
        input_ids = tokenizer(question, return_tensors="pt").input_ids
        dssd_out, *_ = generate_DSSD(uav_node, bs_node, input_ids, tokenizer, args)
        dssd_text = to_text(dssd_out, tokenizer)
        dssd_pred = extract_bool_answer(dssd_text)
        if dssd_pred == gt:
            correct_dssd += 1

        # ===== DSP =====
        input_ids = tokenizer(question, return_tensors="pt").input_ids
        dsp_out, *_ = generate_DSD(uav_node, bs_node, input_ids, tokenizer, args)
        dsp_text = to_text(dsp_out, tokenizer)
        dsp_pred = extract_bool_answer(dsp_text)
        if dsp_pred == gt:
            correct_dsp += 1

    print("\n=== Mini-BoolQ Accuracy ===")
    print(f"Baseline accuracy : {correct_base}/{n_samples} = {correct_base/n_samples:.3f}")
    print(f"DSSD accuracy     : {correct_dssd}/{n_samples} = {correct_dssd/n_samples:.3f}")
    print(f"DSP accuracy      : {correct_dsp}/{n_samples} = {correct_dsp/n_samples:.3f}")

    return {
        "baseline": correct_base / n_samples,
        "dssd": correct_dssd / n_samples,
        "dsp": correct_dsp / n_samples
    }

def to_text(output, tokenizer):
    """将 normal_generate / DSSD / DSP 输出统一转换成字符串"""
    # 情况1：output 是 tuple，例如 (tensor, time, tp)
    if isinstance(output, tuple):
        output = output[0]

    # 情况2：tensor（token ids）
...
def load_mini_gsm8k(n_samples=200, seed=42):
    gsm8k = load_dataset(
        path="/home/lym/sijia/DSSD-Efficient-Edge-Computing/benchmark/gsm8k_local",
        name="main"
    )
    test_set = list(gsm8k["test"])

    random.seed(seed)
    mini = random.sample(test_set, n_samples)
    return mini
...
def evaluate_on_mini_gsm8k(uav_node, bs_node, tokenizer, args, n_samples=50):
    """
    使用 Mini-GSM8K 测试你的 speculative decoding 实现的正确性。
    只取 50 条即可快速回归验证。
    """

    dataset = load_mini_gsm8k(n_samples=n_samples)
    correct_dssd = 0
    correct_dsp = 0
    correct_base = 0

    print("\n=== Running Mini-GSM8K correctness test ===")

    for item in dataset:
        q = item["question"]
        gt = extract_answer(item["answer"])  # GSM8K answer 格式多为 "#### 42"

        # ===== baseline =====
        input_ids = tokenizer(q, return_tensors="pt").input_ids.to(args.device_2)
        base_out, _, _ = normal_generate(bs_node.model, tokenizer, input_ids, args.device_2, args)
        base_text = to_text(base_out, tokenizer)
        ...
        dssd_out, *_ = generate_DSSD(uav_node, bs_node, input_ids, tokenizer, args)
        dssd_pred = extract_answer(dssd_out)
        if dssd_pred == gt:
            correct_dssd += 1
        ...
        input_ids = tokenizer(q, return_tensors="pt").input_ids
        dsp_out, *_ = generate_DSD(uav_node, bs_node, input_ids, tokenizer, args)
        dsp_pred = extract_answer(dsp_out)
        if dsp_pred == gt:
            correct_dsp += 1

    # === 输出测试结果 ===
    print("\n=== Mini-GSM8K Accuracy ===")
    print(f"Baseline LLM accuracy : {correct_base}/{n_samples} = {correct_base/n_samples:.3f}")
    print(f"DSSD accuracy         : {correct_dssd}/{n_samples} = {correct_dssd/n_samples:.3f}")
    print(f"DSP  accuracy         : {correct_dsp}/{n_samples} = {correct_dsp/n_samples:.3f}")

    # 如果 speculative decoding 正确，这三者应几乎一致（差异 < 1%）
    return {
        "baseline": correct_base / n_samples,
        "dssd": correct_dssd / n_samples,
        "dsp": correct_dsp / n_samples
...
if __name__ == "__main__":
    ...
    # 运行 Mini-GSM8K 回归测试
    ##########################################
    # print("\n===== Running Mini-GSM8K regression test =====")
    # res = evaluate_on_mini_gsm8k(
    #     uav_node=uav_node,
    #     bs_node=bs_node,
    #     tokenizer=tokenizer,
    #     args=args,
    #     n_samples=50   # 可以改为 100–300
    # )
    # print("Mini-GSM8K test results:", res)

    print("\n===== Running Mini-BoolQ regression test =====")
    res = evaluate_on_mini_boolq(
        uav_node=uav_node,
        bs_node=bs_node,
        tokenizer=tokenizer,
        args=args,
